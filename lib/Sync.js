...
exports.isSyncableType = isSyncableType;
exports.createSyncData = createSyncData;
exports.syncTo = syncTo;
exports.lateBindMember = lateBindMember;
...
function isSyncable(obj) {
    ...
    if (obj instanceof Array) return obj.every(val => isSyncable(val));
 ...
 */
function createSyncData(aObject) {
    ...
    return Object.keys(items).map(item => {
        ...
        if (itemType === _SyncDecorators.LATE_BIND) throw new Error("All late bound members must be bound before creating sync data");
        if (isSyncable(value)) {
            if (isArray) {
                value = value.map(item => {
                    if (item.toSyncData) return item.toSyncData();
 ...
 */
function createElem(obj, itemType, elemValue) {
    if (itemType === _SyncDecorators.PRIMITIVE || itemType === _SyncDecorators.LATE_BIND) {
        return elemValue;
    } else if (elemValue) {
        //if the element type is not a primitive, create the element via the sync provider
        const syncProvider = Object.getPrototypeOf(obj).syncProvider;
        if (!syncProvider) throw new Error("the object " + JSON.stringify(obj) + " does not contain a sync provider, SYNC FAILED");
        // get the ctor name of type that should be created
        const ctorName = itemType.name;
        if (!ctorName) throw new Error("item name is not a function i.e. not a ctor given type was:" + itemType);
        const objInstance = syncProvider.create(ctorName);
...
function syncTo(obj, syncData) {
    ...
    syncData.forEach(elem => {
        ...
        if (type == _SyncDecorators.LATE_BIND) throw new Error("All late bound members must be bound before syncing");
 ...
 */
function lateBindMember(obj, name, type) {
    const proto = Object.getPrototypeOf(obj);
    if (!proto.syncItems || !isSyncable(obj)) throw new Error("Object is not syncable, is it annotated with @autoSync?");
    const syncElem = proto.syncItems[name];
    if (!syncElem.allowLateBinding) return;
    if (syncElem) {
        if (syncElem.type === _SyncDecorators.LATE_BIND) {
            syncElem.type = type;
 ...
 */
function syncListItems(authority, clients) {
    const syncData = authority.toSyncData();
    clients.forEach(client => {
        if (client !== authority) client.syncFrom(syncData);
    });
}